...
namespace QuizForam.App_Data
{
    ...
    public static class DAL
    {
        ...
        {
            object DoStored;
            using (SqlCommand myCommand = new SqlCommand(sqlStored, (transactional) ? GetConnection : GetNewConnection))
            {
        ...
        {
            bool isTransaction = IsTransaction;
            using (SqlCommand myCommand = new SqlCommand(sqlQuery, (isTransaction) ? GetConnection : GetNewConnection))
            {
                try
                {
                    foreach (DbSqlParameter p in sqlParams)
                ...
                {
                    LogFileWrite("Page Url:   " + HttpContext.Current.Request.Url + Environment.NewLine + "In Dal layer  (SqlScalartoObj Two Para) " + Environment.NewLine + "Error  : " + err.Message);
                    return null;
                }
                finally
                {
